...
class dki4wp_Admin {
     ...
     *
     * @since    1.0.0
     * @param      string    $dki4wp       The name of this plugin.
    ...
    public function get_sets_config() {
        return base64_encode( '{"sets":2,"params":3}' );
    }

    public function enqueue_styles( $hook_suffix ) {
        if ( in_array( $hook_suffix, array('toplevel_page_dynamic-keyword-insertion-for-wp') ) ) {
            wp_enqueue_style(
                $this->dki4wp . '-generator-app-css',
                plugin_dir_url( __FILE__ ) . 'css/generator.app.min.css',
                array(),
                $this->version,
                'all'
            );
        }
        if ( in_array( $hook_suffix, array('toplevel_page_dynamic-keyword-insertion-for-wp', 'dynamic-content_page_dynamic-keyword-insertion-for-wp-sets') ) ) {
            wp_enqueue_style(
                $this->dki4wp,
                plugin_dir_url( __FILE__ ) . 'css/dynamic-keyword-insertion-for-wp-admin.css',
                array(),
                $this->version,
                'all'
            );
    ...
    public function enqueue_scripts( $hook_suffix ) {
        if ( in_array( $hook_suffix, array('toplevel_page_dynamic-keyword-insertion-for-wp') ) ) {
            wp_enqueue_script(
                $this->dki4wp . '-generator-chunk',
                plugin_dir_url( __FILE__ ) . 'js/generator.chunk-vendors.min.js',
                array('jquery'),
                $this->version,
                true
            );
            wp_enqueue_script(
                $this->dki4wp . '-generator-app',
                plugin_dir_url( __FILE__ ) . 'js/generator.app.min.js',
                array('jquery'),
                $this->version,
                true
            );
        ...
        if ( in_array( $hook_suffix, array('dynamic-content_page_dynamic-keyword-insertion-for-wp-sets') ) ) {
            wp_enqueue_script(
                $this->dki4wp . '-sets-chunk',
                plugin_dir_url( __FILE__ ) . 'js/sets.chunk-vendors.min.js',
                array('jquery'),
                $this->version,
                true
            );
            wp_enqueue_script(
                $this->dki4wp . '-sets-app',
                plugin_dir_url( __FILE__ ) . 'js/sets.app.min.js',
                array('jquery'),
                $this->version,
                true
            );
     ...
     * @since    1.0.0
     */
    public function dki4wp_settings_init() {
        // Register a new setting for "dki4wp_sets" page.
        register_setting( 'dki4wp_sets', 'dki4wp_sets_data' );
        // Register a new section in the "dki4wp_sets" page.
        add_settings_section(
            'dki4wp_section_sets',
            null,
            array($this, 'dki4wp_section_sets_callback'),
            'dki4wp_sets'
        );
        // Register a new field in the "dki4wp_section_sets" section, inside the "dki4wp_sets" page.
        add_settings_field(
            'dki4wp_sets_sets',
            __( 'dynamic-keyword-insertion-for-wp', 'dynamic-keyword-insertion-for-wp' ),
            array($this, 'dki4wp_sets_sets_cb'),
            'dki4wp_sets',
            'dki4wp_section_sets',
            array(
                'label_for'          => 'dki4wp_sets_sets',
                'index'              => 0,
                'class'              => 'dki4wp_row',
                'dki4wp_custom_data' => 'custom',
            )
        );
    }
    ...
    public function add_menu_pages() {
        ...
        add_submenu_page(
            ...
            'Sets',
            'manage_options',
            'dynamic-keyword-insertion-for-wp-sets',
            array($this, 'dki4wp_sets_page_html')
        );
    }
     ...
     * @since    1.0.0
     */
    public function dki4wp_page_html() {
        global $dki4wp_fs;
        ...
        if ( isset( $_GET['settings-updated'] ) ) {
            ...
            add_settings_error(
                'dki4wp_messages',
                'dki4wp_message',
                __( 'Settings Saved', 'dynamic-keyword-insertion-for-wp' ),
                'updated'
            );
        }
        // show error/update messages
        settings_errors( 'dki4wp_messages' );
        ?>
        ...
        echo esc_url( plugin_dir_url( __FILE__ ) . '/json/start.json' );
        ?>"></div>
            </div>
            <div class="wrap-aside-panel">
                <h3>Integration with SEO Plugins</h3>
                <div class="feed" data-bvnode-plugin="<?php 
        echo esc_url( plugin_dir_url( dirname( __FILE__ ) ) );
        ?>"
                    data-bvnode-feed="<?php 
        echo esc_url( plugin_dir_url( __FILE__ ) . '/json/seo.json' );
        ?>"></div>
            </div>
            <div class="wrap-aside-panel">
                <h3>How to use</h3>
        ...
        ?>"
                    data-bvnode-feed="<?php 
        echo esc_url( plugin_dir_url( __FILE__ ) . '/json/howto.json' );
        ?>"></div>
            </div>
            <?php 
    ...
    /**
     * Sets Settings Page HTML
     *
     * @since    1.0.0
     */
    public function dki4wp_sets_page_html() {
        // check user capabilities
        if ( !current_user_can( 'manage_options' ) ) {
            return;
        }
        ...
        settings_errors( 'dki4wp_messages' );
        ?>
        <div class="wrap wrap--bvnode wrap--bvnode-sets">
            <?php 
        $this->page_header();
        ?>
            <h1>
        ...
        ?>
            ...
            <form action="options.php" method="post">
                <?php 
        // output security fields for the registered setting "dki4wp_sets"
        settings_fields( 'dki4wp_sets' );
        // output setting sections and their fields
        do_settings_sections( 'dki4wp_sets' );
        ...
        ?>
            <div class="wrap-aside-panel">
                <h3>Learn about Sets</h3>
                <div class="feed" data-bvnode-plugin="<?php 
        echo esc_url( plugin_dir_url( dirname( __FILE__ ) ) );
        ?>"
                    data-bvnode-feed="<?php 
        echo esc_url( plugin_dir_url( __FILE__ ) . '/json/sets.json' );
        ?>"></div>
            </div>
            <div class="wrap-aside-panel">
                <h3>Integration with SEO Plugins</h3>
    ...
    public function dki4wp_section_sets_callback( $args ) {
        return null;
        ?>
        <p id="<?php 
        ...
        ?>">
            <?php 
        esc_html_e( 'Define sets of parameters that are comonly used together, you can select this sets as default value on certain page you need.', 'dynamic-keyword-insertion-for-wp' );
        ?>
        </p>
        <?php 
    }

    public function dki4wp_sets_sets_cb( $args ) {
        // Get the value of the setting we've registered with register_setting()
        $options = get_option( 'dki4wp_sets_data' );
        ?>

        <div data-sets data-config="<?php 
        echo esc_html( $this->get_sets_config() );
        ?>"></div>
        ...
        ?>").innerText = window.setsApp.getSetsValue();
                window.setsApp.setSubmit();
            })
        </script>
        <textarea id="<?php 
        echo esc_attr( $args['label_for'] );
        ?>"
            name="dki4wp_sets_data[<?php 
        echo esc_attr( $args['label_for'] );
        ?>]"
            style=" display: none;"><?php 
        echo esc_html( ( !empty( $options[$args['label_for']] ) ? $options[$args['label_for']] : '{}' ) );
        ?></textarea>
